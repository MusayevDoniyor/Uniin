...
function OnboardingPage() {
  ...
  const finish = async () => {
    if (form.phone && !isValidUzPhone(form.phone))
      return toast.error("Please enter a valid Uzbek phone number");
    setSubmitting(true);
    const { error } = await supabase
      ...
      .update({
        ...
        gpa: form.gpa_na || !form.gpa ? null : parseFloat(form.gpa),
        ...
        dream_universities: form.dream_universities,
    ...
    if (isGU && form.gu_unis.length && profile.id) {
      await supabase.from("gu_universities").insert(
        form.gu_unis.map((u, i) => ({
...
function PickType({ onPicked, userId }: { onPicked: () => void; userId: string }) {
  ...
  return (
    <div className="mt-6 flex gap-3">
      <Button onClick={() => pick("gu")} className="bg-gold text-gold-foreground hover:bg-gold/90">
        I'm G.U.
      </Button>
      <Button onClick={() => pick("prep")} className="bg-info hover:bg-info/90">
        I'm Prep
      </Button>
    </div>
  );
...
function GUSteps(props: any) {
  ...
  if (step === 4)
    return (
      <SkillsExtras {...props} bioPrompt="Describe yourself and what you can help others with" />
    );
  return null;
}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Select value={form.grade} onValueChange={(v) => update({ grade: v })}>
              ...
              <SelectContent>
                {["9th", "10th", "11th", "12th", "Gap Year"].map((g) => (
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                update({ target_year: e.target.value ? parseInt(e.target.value) : null })
              }
              className="mt-1.5"
              placeholder="2026"
            />
...
function UniversityPicker({ form, update }: any) {
  ...
  const results = query
    ? UNIVERSITIES.filter((u) => u.name.toLowerCase().includes(query.toLowerCase())).slice(0, 6)
    : [];
  ...
  const addUni = () => {
    ...
    update({
      gu_unis: [
        ...form.gu_unis,
        { ...adding, year_admitted: year ? parseInt(year) : null, degree_type: degree, major },
      ],
    });
    setAdding(null);
    setYear("");
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        This becomes your verified badge across Uniin.
      </p>
      ...
      {form.gu_unis.length > 0 && (
        <div className="space-y-2">
          {form.gu_unis.map((u: any, i: number) => (
            <div
              key={i}
              className="flex items-center justify-between p-3 surface-card gu-accent-border"
            >
              <div>
                <div className="font-semibold flex items-center gap-2">
                  <span className="text-gold">🎓</span> {u.university_name}
                </div>
                <div className="text-xs text-muted-foreground mt-0.5">
                  {u.country} · {u.degree_type} {u.major && `in ${u.major}`}{" "}
                  {u.year_admitted && `· ${u.year_admitted}`}
                </div>
              </div>
              <Button
                size="sm"
                variant="ghost"
                onClick={() =>
                  update({ gu_unis: form.gu_unis.filter((_: any, j: number) => j !== i) })
                }
              >
                <X className="size-4" />
              </Button>
      ...
      {!adding ? (
        <div>
          ...
          <div className="relative mt-1.5">
            ...
            <Input
              ...
              className="pl-9"
              placeholder="e.g. MIT, Harvard, Oxford..."
            />
          </div>
          {results.length > 0 && (
            <div className="mt-2 surface-card divide-y divide-border max-h-64 overflow-auto">
              {results.map((u) => (
                <button
                  key={u.name}
                  onClick={() =>
                    setAdding({ university_name: u.name, country: u.country, qs_rank: u.qsRank })
                  }
                  className="w-full text-left p-3 hover:bg-surface-2 flex justify-between items-center"
                >
                  <div>
        ...
        <div className="surface-card p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button className="flex-1 bg-primary" onClick={addUni}>
              Add university
            </Button>
          </div>
        </div>
...
function AcademicStats({ form, update, prep }: any) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        {prep
          ? "Where do you stand today? You can mark anything you haven't taken yet."
          : "Your record speaks for you. Toggle off what doesn't apply."}
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
        ...
        <div>
          ...
          <Select
            value={String(form.gpa_scale)}
            onValueChange={(v) => update({ gpa_scale: parseFloat(v) })}
          >
            <SelectTrigger className="mt-1.5">
              <SelectValue />
            </SelectTrigger>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <ScoreInput
          ...
          max={1600}
          placeholder="1500"
          notTaken={form.sat_na}
          onToggleNotTaken={(v) => update({ sat_na: v })}
        />
        <ScoreInput
          label="IELTS"
          value={form.ielts}
          onChange={(v) => update({ ielts: v })}
          min={0}
          max={9}
          step={0.5}
          placeholder="7.5"
          suffix="/9"
          notTaken={form.ielts_na}
          onToggleNotTaken={(v) => update({ ielts_na: v })}
        />
        <ScoreInput
          label="TOEFL"
          value={form.toefl}
          onChange={(v) => update({ toefl: v })}
          min={0}
          max={120}
          placeholder="105"
          suffix="/120"
          notTaken={form.toefl_na}
          onToggleNotTaken={(v) => update({ toefl_na: v })}
        />
      </div>
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1 mb-3">
          Kategoriya bo'yicha tartibga solinadi. Har biriga tafsilot, havola yoki fayl qo'shsangiz
          bo'ladi.
        </p>
        <ExtracurricularUploader
          value={form.extracurricular_items}
          onChange={(v: ECItem[]) => update({ extracurricular_items: v })}
        />
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Tugmalarni bosib tanlang yoki o'zingiznikini qo'shing.
        </p>
        ...
        {form.interests.filter((i: string) => !INTEREST_PRESETS.includes(i)).length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {form.interests
              .filter((i: string) => !INTEREST_PRESETS.includes(i))
              .map((i: string, idx: number) => (
                <span
                  key={idx}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-info/10 text-info text-xs"
                >
                  {i}{" "}
                  <button
                    onClick={() =>
                      update({ interests: form.interests.filter((x: string) => x !== i) })
                    }
                  >
                    <X className="size-3" />
                  </button>
        ...
        <div className="flex flex-wrap gap-2 mt-1.5">
          {COUNTRIES.map((c: any) => (
            <button
              key={c.code}
              type="button"
              onClick={() => toggleArr("target_countries", c.code)}
          ...
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                if (uni.trim()) {
                  update({ dream_universities: [...form.dream_universities, uni.trim()] });
                  setUni("");
                }
              }
            }}
          ...
          {form.dream_universities.map((u: string, idx: number) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  update({
                    dream_universities: form.dream_universities.filter(
                      (_: any, j: number) => j !== idx,
                    ),
                  })
                }
              >
...
function CertsStep({ form, update }: any) {
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Add credentials LinkedIn-style — name, issuer, and date. You can always add more from your
        profile later.
      </p>
      <CertificateEditor
