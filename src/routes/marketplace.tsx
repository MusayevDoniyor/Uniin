...
function Marketplace() {
  ...
  const load = async () => {
    const { data } = await supabase
      ...
      .select(
        "*, profiles!marketplace_listings_seller_id_fkey(id, full_name, avatar_url, user_type)",
      )
      .eq("status", "active")
  ...
  const create = async () => {
    ...
    if (!form.is_free && (!form.price_usd || Number(form.price_usd) <= 0))
      return toast.error("Set a price or mark as free");
      ...
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Resources, mentorship, and packages from verified G.U. students.
          </p>
        </div>
              ...
              <div className="space-y-4">
                    ...
                    {form.cover_image_url ? (
                      <div className="relative h-40 rounded-lg overflow-hidden border border-border">
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 size-7 rounded-full bg-background/80 flex items-center justify-center"
                        >
                          <X className="size-4" />
                        </button>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      className="mt-1.5"
                      placeholder="MIT Common App essay — admitted 2024"
                    />
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setForm((f) => ({ ...f, listing_type: v }))}
                    >
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className="mt-1.5"
                    placeholder="What makes this valuable for prep students?"
                  />
                  <span className="text-[10px] text-muted-foreground">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    maxLength={300}
                    rows={2}
                    onChange={(e) => setForm((f) => ({ ...f, what_included: e.target.value }))}
                    className="mt-1.5"
                    placeholder="• Full essay&#10;• Brainstorm notes&#10;• 30-min call"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="sm:col-span-2 flex items-end gap-3">
                    <div className="flex-1">
                      ...
                      <Input
                        ...
                        value={form.is_free ? "" : form.price_usd}
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setForm((f) => ({ ...f, delivery_time: v }))}
                    >
                      <SelectTrigger className="mt-1.5">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {["Instant", "1 day", "3 days", "1 week", "2 weeks"].map((d) => (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {form.tags.map((t, i) => (
                      <span
                        key={t}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-surface-2 text-xs"
                      >
                        #{t}
                        <button
                          type="button"
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    rows={3}
                    onChange={(e) => setForm((f) => ({ ...f, preview_content: e.target.value }))}
                    className="mt-1.5"
                    placeholder="The opening paragraph or sample chunk..."
                  />
                </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
        {[
          ...
          return (
            <button
              key={c.v}
              onClick={() => setFilter(active ? "all" : c.v)}
      ...
      <div className="flex gap-1.5 overflow-x-auto mb-4 -mx-1 px-1 pb-1">
        {[{ v: "all", l: "All" }, ...TYPES].map((t) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs whitespace-nowrap border transition-colors ${filter === t.v ? "bg-primary border-primary text-primary-foreground" : "border-border hover:border-primary/50 text-muted-foreground"}`}
          >
            {t.l}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((l) => (
          <button
            key={l.id}
            onClick={() => setActive(l)}
            className="surface-card overflow-hidden text-left hover:border-primary/40 transition-colors group"
          >
            {l.cover_image_url ? (
            ...
            <div className="p-4">
              <div className="flex items-center gap-2 mb-2">
                <Avatar className="size-6">
                  ...
                  <AvatarFallback>{l.profiles?.full_name?.[0]}</AvatarFallback>
                </Avatar>
              ...
              <div className="flex justify-between items-center mt-3">
                <span className="text-[10px] px-2 py-0.5 rounded bg-surface-2 capitalize">
                  {l.listing_type.replace("_", " ")}
                </span>
                <span className="font-bold text-primary">
                  {l.is_free ? "Free" : `$${l.price_usd}`}
                </span>
              </div>
            </div>
          </button>
          ...
          {active && (
            <>
              {active.cover_image_url && (
                <div className="h-40 -m-6 mb-2 overflow-hidden">
                  <img
              ...
              <div className="flex items-center gap-2 text-sm">
                <Avatar className="size-7">
                  <AvatarImage src={active.profiles?.avatar_url} />
                  <AvatarFallback>{active.profiles?.full_name?.[0]}</AvatarFallback>
                </Avatar>
              ...
              <div className="flex flex-wrap gap-3 text-xs text-muted-foreground mt-2">
                {active.delivery_time && (
                  <span className="flex items-center gap-1">
                    <Clock className="size-3" /> {active.delivery_time}
                  </span>
                )}
