  ...
  const save = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        chat_rate_usd:
          profile.is_premium && profile.user_type === "gu" && f.chat_rate_usd
            ? parseFloat(f.chat_rate_usd)
            : null,
        call_rate_usd:
          profile.is_premium && profile.user_type === "gu" && f.call_rate_usd
            ? parseFloat(f.call_rate_usd)
            : null,
        booking_rate_usd:
          profile.is_premium && profile.user_type === "gu" && f.booking_rate_usd
            ? parseFloat(f.booking_rate_usd)
            : null,
      } as any)
      .eq("user_id", user.id);
    setSaving(false);
      ...
      <Tabs defaultValue="profile">
        ...
        <TabsContent value="profile" className="space-y-4 mt-4">
          <Section title="Cover photo">
            <div
              className="relative h-40 rounded-lg overflow-hidden border border-border bg-surface-2"
              style={
                f.cover_image_url
                  ? {
                      backgroundImage: `url(${f.cover_image_url})`,
                      backgroundSize: "cover",
                      backgroundPosition: "center",
                    }
                  : {
                      background:
                        "linear-gradient(135deg, oklch(0.22 0.06 265), oklch(0.40 0.15 27))",
                    }
              }
            >
          ...
          <Section title="Profile photo">
            ...
            <p className="text-xs text-muted-foreground mt-3">
              JPG or PNG, up to ~5MB. Or pick one from our default collection.
            </p>
          </Section>
              ...
              <div>
                <Label>
                  ...
                  <span className="text-xs text-muted-foreground font-normal">
                    (profil havolasi: /profile/@{f.username || "—"})
                  </span>
                </Label>
                <Input
                  maxLength={30}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <Label className="flex items-center gap-1.5">
                        <MessageSquare className="size-3.5" /> Chat (per thread)
                      </Label>
                      <div className="relative mt-1.5">
                        ...
                        <Input
                          ...
                          className="pl-7"
                          placeholder="0"
                          value={f.chat_rate_usd}
                          onChange={(e) => update({ chat_rate_usd: e.target.value })}
                        />
                      </div>
                    </div>
                    <div>
                      <Label className="flex items-center gap-1.5">
                        <Phone className="size-3.5" /> Call (per session)
                      </Label>
                      <div className="relative mt-1.5">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">
                          $
                        ...
                        <Input
                          ...
                          className="pl-7"
                          placeholder="0"
                          value={f.call_rate_usd}
                          onChange={(e) => update({ call_rate_usd: e.target.value })}
                        />
                      </div>
                    </div>
                    <div>
                      <Label className="flex items-center gap-1.5">
                        <CalendarClock className="size-3.5" /> Booking (per session)
                      </Label>
                      <div className="relative mt-1.5">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">
                          $
                        ...
                        <Input
                          ...
                          className="pl-7"
                          placeholder="0"
                          value={f.booking_rate_usd}
                          onChange={(e) => update({ booking_rate_usd: e.target.value })}
                        />
                      </div>
                    </div>
                  </div>
                  <p className="text-[11px] text-muted-foreground mt-2">
                    20% platform fee · funds released after 7 days via escrow.
                  </p>
                </>
              ) : (
                <div className="rounded-lg border border-gold/30 bg-gold/5 p-4 flex flex-wrap items-center justify-between gap-3">
                  <div className="text-sm">
                    <div className="font-semibold flex items-center gap-1.5">
                      <Crown className="size-4 text-gold" /> Pullik service'lar Premium'da
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Chat, call va booking uchun narx qo'yish faqat Premium G.U. uchun.
                    </p>
                  </div>
                  <Link
                    to="/premium"
                    className="px-3 py-1.5 rounded-md bg-gold text-gold-foreground text-xs font-semibold hover:bg-gold/90"
                  >
                    Upgrade
                  </Link>
                </div>
        ...
        <TabsContent value="academic" className="space-y-4 mt-4">
          <Section title="Test scores">
            <p className="text-xs text-muted-foreground mb-4">
              Toggle "Don't have" for anything you haven't taken yet — you can always add it later.
            </p>
          ...
          <Section title="Boshqa imtihonlar / ballar">
            <CustomStatsEditor
              value={f.custom_stats}
              onChange={(v) => update({ custom_stats: v })}
            />
          </Section>
        </TabsContent>
        ...
        <TabsContent value="extras" className="space-y-4 mt-4">
          <Section title="Faoliyatlar (Extracurriculars)">
            <p className="text-xs text-muted-foreground mb-3">
              O'zingiz qilgan ishlarni kategoriya bo'yicha yuklang. Fayllar (rasm/PDF) va havolalar
              qo'shsa bo'ladi.
            </p>
            <ExtracurricularUploader
              value={f.extracurricular_items}
              onChange={(v) => update({ extracurricular_items: v })}
            />
          </Section>
        </TabsContent>
        ...
        <TabsContent value="certs" className="space-y-4 mt-4">
          <Section title="Certifications">
            <p className="text-xs text-muted-foreground mb-3">
              LinkedIn-style. Add credentials with name, issuer, date, link, and (optional)
              certificate image.
            </p>
            <CertificateEditor
          ...
          <Section title="Theme">
            <div className="grid grid-cols-3 gap-3">
              <ThemeCard
                ...
                icon={<Sun className="size-5 text-gold" />}
                label="Light"
                sub="Crisp & bright"
              />
              <ThemeCard
                ...
                icon={<Moon className="size-5 text-info" />}
                label="Dark"
                sub="Easy on eyes"
              />
              <ThemeCard
