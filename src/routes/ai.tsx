  ...
  const send = async (overrideMode?: Mode, overrideInput?: string) => {
    ...
    const tools: Record<Mode, any> = {
      university_match: {
        ...
        description: "Return university recommendations.",
        parameters: {
          type: "object",
          properties: {
            summary: { type: "string" },
            universities: {
              ...
              items: {
                ...
                properties: {
                  ...
                  match_level: { type: "string", enum: ["Safety", "Match", "Reach"] },
      ...
      profile_analyzer: {
        ...
        description: "Analyze the profile.",
        parameters: {
          type: "object",
          properties: {
            summary: { type: "string" },
...
- Bio: ${profile.bio}`;
  ...
  const STARTERS = [
    {
      m: "university_match" as Mode,
      icon: Target,
      title: "Find universities that match my profile",
      text: "Suggest universities for me based on my profile.",
    },
    {
      m: "profile_analyzer" as Mode,
      icon: BarChart3,
      title: "Analyze my profile — what am I missing?",
      text: "Analyze my profile and tell me what gaps I have.",
    },
    {
      m: "essay_coach" as Mode,
      icon: PenLine,
      title: "Coach me on my essay",
      text: "Coach me. Here's my essay: [paste your essay here]",
    },
  ];
      ...
      <div className="lg:w-[55%] flex flex-col border-r border-border">
        <div className="p-4 border-b border-border">
          ...
          <Tabs value={mode} onValueChange={(v) => setMode(v as Mode)}>
            <TabsList className="w-full">
              <TabsTrigger value="university_match" className="flex-1">
                <Target className="size-4 mr-1.5" />
                Match
              </TabsTrigger>
              <TabsTrigger value="profile_analyzer" className="flex-1">
                <BarChart3 className="size-4 mr-1.5" />
                Analyze
              </TabsTrigger>
              <TabsTrigger value="essay_coach" className="flex-1">
        ...
        <div className="p-3 border-t border-border flex gap-2">
          ...
          <Button
            onClick={() => send()}
            disabled={loading}
            className="bg-primary hover:bg-accent self-end"
          >
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex gap-2">
        <span className="px-3 py-1.5 rounded-full bg-success/15 text-success text-xs font-semibold">
          {counts.Safety} Safety
        </span>
        <span className="px-3 py-1.5 rounded-full bg-info/15 text-info text-xs font-semibold">
          {counts.Match} Match
        </span>
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground mt-0.5">
                  {u.country} {u.acceptance_rate && `· ${u.acceptance_rate}% acceptance`}{" "}
                  {u.deadline && `· ${u.deadline}`}
                </div>
              </div>
              <div className="text-right">
                <span
                  className={`px-2 py-0.5 text-xs rounded-full font-semibold ${u.match_level === "Safety" ? "bg-success/15 text-success" : u.match_level === "Match" ? "bg-info/15 text-info" : "bg-primary/15 text-primary"}`}
                >
                  {u.match_level}
                </span>
      ...
      {data.next_steps?.length > 0 && (
        <div className="surface-card p-4">
          <h3 className="font-semibold mb-2 flex items-center gap-2">
            <TrendingUp className="size-4 text-primary" /> Next steps
          </h3>
    ...
    <div className="space-y-4 max-w-2xl">
      <div className="surface-card p-6 flex items-center gap-6">
        <div className="size-24 rounded-full border-4 border-primary flex items-center justify-center">
          <div>
            <div className="text-3xl font-bold">{data.overall_score}</div>
            <div className="text-[10px] text-muted-foreground text-center">/100</div>
          </div>
        </div>
        <div className="flex-1">
          <div className="text-xs text-muted-foreground">Overall Profile Score</div>
          <div className="font-semibold mt-1">{data.verdict}</div>
        </div>
      </div>
      ...
      {data.strong_points?.length > 0 && (
        <div className="surface-card p-4">
          ...
          <div className="space-y-1.5">
            {data.strong_points.map((p: string, i: number) => (
              <div key={i} className="flex gap-2 text-sm">
                <CheckCircle2 className="size-4 text-success shrink-0 mt-0.5" /> {p}
              </div>
            ))}
          </div>
        </div>
            ...
            <div key={i} className="surface-card p-4">
              <div className="flex justify-between items-start">
                <div className="font-semibold flex items-center gap-2">
                  <AlertTriangle className="size-4 text-gold" /> {g.area}
                </div>
                <span
                  className={`text-xs px-2 py-0.5 rounded ${g.priority === "High" ? "bg-primary/20 text-primary" : "bg-gold/20 text-gold"}`}
                >
                  {g.priority}
                </span>
              </div>
              <div className="text-xs text-muted-foreground mt-1">
                Now: {g.current} → Target: {g.target}
              </div>
    ...
    <div className="space-y-4 max-w-2xl">
        ...
        <div className="surface-card p-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {data.strengths.map((s: string, i: number) => (
              <span key={i} className="px-2 py-1 text-xs rounded bg-success/10 text-success">
                {s}
              </span>
            ))}
          </div>
      ...
      {data.weaknesses?.map((w: any, i: number) => (
        <div key={i} className="surface-card p-4">
          ...
          <blockquote className="border-l-2 border-primary pl-3 my-2 text-sm italic text-muted-foreground">
            "{w.quoted_text}"
          </blockquote>
      ...
      <div className="flex gap-2 text-xs">
        ...
        {data.word_count_note && (
          <span className="px-2 py-1 rounded bg-surface-2">{data.word_count_note}</span>
        )}
      </div>
    </div>
  );
