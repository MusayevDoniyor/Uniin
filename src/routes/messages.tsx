  ...
  return (
    <div className="flex h-[calc(100vh-65px)]">
      <aside className="w-72 border-r border-border overflow-y-auto">
        <div className="p-4 border-b border-border"><h2 className="font-bold">Messages</h2></div>
        ...
        {convs.map(c => (
          <button key={c.id} onClick={() => setActiveId(c.id)} className={`w-full text-left p-3 flex gap-3 hover:bg-surface ${activeId === c.id ? "bg-surface" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        {activeId ? <Conversation id={activeId} conv={convs.find(c => c.id === activeId)} /> : <div className="h-full flex items-center justify-center text-muted-foreground">Pick a conversation</div>}
      </div>
    </div>
  );
}

function Conversation({ id, conv }: { id: string; conv: any }) {
  const { user } = useAuth();
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="p-4 border-b border-border flex items-center gap-3">
        ...
        <div className="font-semibold">{conv?.other?.full_name || "—"}</div>
      </header>
        ...
        {msgs.map(m => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[70%] rounded-2xl px-4 py-2 text-sm ${mine ? "bg-primary text-primary-foreground" : "bg-surface"}`}>
                {m.content}
