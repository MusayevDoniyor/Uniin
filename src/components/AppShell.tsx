...
export function AppShell({ children, rightSidebar }: { children: React.ReactNode; rightSidebar?: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`shell-${user.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications", filter: `user_id=eq.${user.id}` }, (payload: any) => {
        ...
        if (msg) toast(msg, { description: "New notification", action: { label: "View", onClick: () => navigate({ to: "/notifications" }) } });
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "notifications", filter: `user_id=eq.${user.id}` }, loadN)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "wallets", filter: `user_id=eq.${user.id}` }, loadW)
      .subscribe();
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center gap-2 rounded-full hover:bg-surface p-1 pr-2 transition-colors">
          <Avatar className="size-8"><AvatarImage src={profile?.avatar_url || undefined} /><AvatarFallback>{profile?.full_name?.[0] || "U"}</AvatarFallback></Avatar>
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-60">
        <DropdownMenuLabel className="flex items-center gap-2">
          <Avatar className="size-9"><AvatarImage src={profile?.avatar_url || undefined} /><AvatarFallback>{profile?.full_name?.[0]}</AvatarFallback></Avatar>
        ...
        <DropdownMenuItem onClick={() => navigate({ to: "/profile/$id", params: { id: profileHref } })}><User className="size-4 mr-2" /> Profile</DropdownMenuItem>
        ...
        <DropdownMenuItem onClick={() => navigate({ to: "/wallet" })}><Wallet className="size-4 mr-2" /> Wallet</DropdownMenuItem>
        <DropdownMenuItem onClick={() => navigate({ to: "/premium" })}><Crown className="size-4 mr-2" /> Premium</DropdownMenuItem>
  ...
  const WalletChip = (
    <Link to="/wallet" className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-2 hover:bg-surface text-xs font-semibold border border-border">
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      <div className="flex-1 flex min-w-0">
        <main className="flex-1 min-w-0">
          ...
          <header className="md:hidden flex items-center justify-between gap-2 p-3 border-b border-border sticky top-0 bg-background/90 backdrop-blur z-20">
            <Link to="/feed"><Logo size="md" /></Link>
            <div className="flex items-center gap-1">
              <button onClick={() => setMobileSearchOpen(v => !v)} className="p-2 rounded-lg hover:bg-surface"><Search className="size-5" /></button>
              <Link to="/notifications" className="relative p-2 rounded-lg hover:bg-surface">
                <Bell className="size-5" />
                {unread > 0 && <span className="absolute top-1 right-1 size-4 rounded-full bg-primary text-primary-foreground text-[10px] flex items-center justify-center font-semibold">{unread}</span>}
              </Link>
              <Link to="/wallet" className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-surface-2 text-[11px] font-semibold border border-border">
          ...
          {mobileSearchOpen && (
            <div className="md:hidden p-3 border-b border-border bg-background sticky top-[57px] z-20"><GlobalSearch /></div>
          )}
        ...
        {rightSidebar && <aside className="hidden xl:block w-72 shrink-0 border-l border-border p-4 sticky top-0 h-screen overflow-y-auto">{rightSidebar}</aside>}
      </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 border-t border-border bg-background/95 backdrop-blur z-30 flex items-end justify-around px-2 pt-1 pb-[max(0.25rem,env(safe-area-inset-bottom))]">
        ...
        <TabLink to="/explore" label="Explore" icon={Compass} active={isActive("/explore")} />
        <button onClick={openComposer} aria-label="New post"
        ...
        <TabLink to="/ai" label="AI" icon={Sparkles} active={isActive("/ai")} />
        <TabLink to="/profile/$id" params={{ id: profileHref }} label="Profile" icon={User} active={loc.pathname.startsWith("/profile")} />
      </nav>
...
function TabLink({ to, params, label, icon: Icon, active }: { to: string; params?: any; label: string; icon: any; active: boolean }) {
  return (
    <Link to={to as any} params={params as any} className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-md transition-colors ${active ? "text-primary" : "text-muted-foreground"}`}>
